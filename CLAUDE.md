...
This is a Python project called `refinire-tool-tavily` that provides a web search API tool using the Tavily service for RefinireAgent. The project is designed to be a tool/plugin that integrates with the Refinire agent system.

## Key Architecture

- **src/refinire_tool_tavily/**: Main package directory
  - **api.py**: Public API functions (search_web, get_search_context)
  - **models.py**: Pydantic data models for validation
  - **service.py**: Tavily service implementation
- **examples/**: Usage examples and demonstrations
- **tests/**: Test suite with comprehensive coverage
- The tool uses environment variables for configuration and provides clean API functions

## Development Commands

### Setup
1. Install package in development mode: `source .venv/bin/activate && uv pip install -e .`
2. Generate environment template: `oneenv template` (creates grouped .env.example)
3. Copy and edit: `cp .env.example .env` and add your Tavily API key
4. Alternative setup: `python examples/setup_example.py`
...
### Project Structure
- Built with Python 3.10+ using modern tooling (uv/pyproject.toml)
- Minimal dependencies currently (empty dependencies list)
- Documentation in `docs/` directory

## Development Notes

- The project appears to be in early development stage with basic scaffolding
- Intended to implement Tavily web search integration for Refinire agents
- Uses environment variables for API configuration
- Follow the refinire tool decorator pattern for agent integration

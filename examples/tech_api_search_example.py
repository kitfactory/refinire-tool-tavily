...
from refinire_tool_tavily import refinire_web_search_programming
...
def main():
    ...
    print("-" * 30)
    try:
        result = refinire_web_search_programming("REST API authentication methods", max_results=3)
        if result["success"]:
    ...
    print("-" * 35)
    try:
        result = refinire_web_search_programming("Python requests library tutorial", max_results=3)
        if result["success"]:
            if "answer" in result:
                print("AI Summary:")
                print(result["answer"])
    ...
    print("-" * 27)
    try:
        result = refinire_web_search_programming("FastAPI async database", max_results=3)
        if result["success"]:
    ...
    print("-" * 25)
    try:
        result = refinire_web_search_programming("Python 非同期処理 async await", max_results=3)
        if result["success"]:
